...
def processData(in_file_name, cust_dict, is_csv = True):
	x_vars_list = []
	y_vars_list = []
	temp = csv.DictReader(in_file_name) if is_csv else in_file_name
	for row in temp:
		...
		if row['fecha_dato'] == '2016-06-28':
			...
			prev_target_list = cust_dict.get(cust_id, [0]*22)
			target_list = getTarget(row)
			new_products = [max(x1 - x2, 0)
			                    for (x1, x2) in zip(target_list, prev_target_list)]
...
if __name__ == "__main__":
	...
	out_df.to_csv(data_path + 'model_results.csv', index=False)
	print(datetime.datetime.now()-start_time)
